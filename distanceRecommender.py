...
class DistanceCollaborativeRecommender:
    ...
    def __init__(self, data, itemNames = None, userNames = None, type_of_recommendation="Item", metric = "pearson"):
        ...
        if (type_of_recommendation == "Item" and not sps.isspmatrix_csc(data)) or (type_of_recommendation == "User" and not sps.isspmatrix_csr(data)):
            raise ValueError(f"For 'type_of_recommendation' equal to '{type_of_recommendation}', 'data' must be a matrix of type {type_of_recommendation}.")

        if data.shape[1] > data.shape[0]:
            raise ValueError("The number of rows should be for 'Users', and the number of columns should be for 'Items'.")
    ...
    def _correlation_pearson_sparse(self, array):
        ...
        xs = np.sqrt(np.add.reduceat((self.data).data**2, (self.data).indptr[:-1]) - (self.data).shape[axis]*xm*xm)

        corr = np.add.reduceat((self.data).data * ys[(self.data).indices], (self.data).indptr[:-1]) / xs
        return corr

    def _search_name(self, search):
        """
    ...
    def _get_recommendation_item(self, item_identification):
        """
        Get item recommendations based on item identification.
        ...
        return self.itemNames[order], similarity[order]
 # Returning the order names and the values
    ...
    def get_recommendation(self, to_recommend, N = 10, need_numeric = False):
        ...
        """
        search_result = self._search_name(to_recommend)
